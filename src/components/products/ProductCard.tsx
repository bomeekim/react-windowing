...
export default function ProductCard(props: IProductCard) {
  return (
    <Card elevation={0} sx={{ cursor: 'pointer' }}>
      ...
      />
      <CardContent
        sx={{ padding: '8px 4px 0 0', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          {props.originPrice !== 0 && (
            <Box display="flex" alignItems="center">
              ...
              <Typography
                ...
                {priceWithComma(props.originPrice)}
              </Typography>
            </Box>
          )}
          <Box display="flex" alignItems="center">
            ...
            <Typography fontSize="16px" lineHeight="18px" fontWeight={600}>
              {priceWithComma(props.finalPrice)}
            </Typography>
          </Box>
        </Box>
        <Box display="flex" alignItems="center" gap="2px">
          {props.badgeList.map((badge, index) => {
            return <img src={badge.image_url} key={index} height={14} alt="badge" />
          })}
        </Box>
        <Box display="flex">
          <Box display="flex" alignItems="center">
          ...
          <Typography variant="caption" fontWeight={500} color="#a1a9ad">
            ({props.reviewCount})
          </Typography>
        </Box>
      </CardContent>
